...
Each infected victim runs a keylogger that sends captured keystrokes to the C2 server over HTTPS POST. The Flask API stores data in PostgreSQL and serves a web dashboard for log analysis.
...
In user mode, a keylogger service receives keystroke events from a kernel-mode keyboard filter driver via IOCTL. The filter driver sits above the i8042prt keyboard driver in the input stack and intercepts IRP_MJ_READ requests before they reach user space.
...
An exfiltration manager selects among channels by priority: webhook, DNS tunnel, email (SMTP), and Dropbox API. Each channel implements a common interface so the manager can fall back when one path is blocked.
