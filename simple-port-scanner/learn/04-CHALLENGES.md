...
Current connect scan: your scanner calls `connect()` and the kernel TCP stack handles the full three-way handshake.

SYN scan: your scanner crafts raw SYN packets (via a raw socket or libnet), sends them directly, and listens for SYN-ACK or RST responses. The kernel is bypassed for sending but still involved for receiving. This requires root privileges and manual packet construction.
...
At the top, a CLI layer handles scan type, timing template, and output format selection. Below that, separate scan engines (SYN, ACK, FIN, and others) share a common packet constructor for TCP header building. The lowest layer combines a timer engine for rate control, raw socket I/O for sending and receiving packets, and optional BPF filtering for response capture.
...
1. **Save your changes** in a new branch
