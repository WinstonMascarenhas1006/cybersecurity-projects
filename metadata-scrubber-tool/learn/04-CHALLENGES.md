...
User-defined profile YAML files are loaded and validated by a profile loader, which passes the parsed rules to each handler so metadata removal rules are applied during the wipe step.
...
Data flows from the cloud storage API through a streaming download, in-memory processing, and a streaming upload back to storage — without writing the full file to local disk.
