...
The anomaly detector runs in three stages. First, a **baseline establishment** phase (learning mode, typically 5–10 minutes) collects normal traffic samples, calculates mean and standard deviation, and stores baseline metrics. Second, **real-time detection** (continuous) compares current traffic against the baseline, calculates Z-scores, and triggers alerts on outliers. Third, an **alert handler** logs anomaly details, displays terminal alerts, and optionally sends webhook notifications.
