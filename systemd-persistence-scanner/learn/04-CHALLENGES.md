...
Each scanner gains a second matching path alongside the existing regex engine: `MatchLine()` continues to handle compiled regex patterns, while a new `MatchYARA()` function applies rules compiled from a user-provided `.yar` file via `yara.Rules`.
