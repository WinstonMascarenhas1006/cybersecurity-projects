...
Add a `VigenereCipher` class to the cipher layer (repeating key with position-dependent shifts) and a `VigenereAnalyzer` to the analysis layer (Kasiski examination and index-of-coincidence calculation).
...
The browser renders frequency charts with Chart.js, sending ciphertext to a Flask backend (`analyzer.py`) via JSON POST requests. The backend computes letter frequencies and returns them for visualization.
